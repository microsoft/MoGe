...
class Baseline(MGEBaselineInterface):
    ...
    def __init__(self, num_tokens: int, resolution_level: int, pretrained_model_name_or_path: str, device: str = 'cuda:0', version: str = 'v1'):
        super().__init__()
    ...
    @staticmethod
    def load(num_tokens: int, resolution_level: int, pretrained_model_name_or_path: str, device: str = 'cuda:0', version: str = 'v1'):
        return Baseline(num_tokens, resolution_level, pretrained_model_name_or_path, device, version)
    ...
    def infer_for_evaluation(self, image: torch.FloatTensor, intrinsics: torch.FloatTensor = None):
        if intrinsics is not None:
            ...
            fov_x = None
        output = self.model.infer(image, fov_x=fov_x, apply_mask=False, num_tokens=self.num_tokens)
